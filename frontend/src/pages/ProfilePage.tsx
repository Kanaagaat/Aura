...
export function ProfilePage() {
  ...
  const TABS = [
    { id: 'active'  as const, label: t('profile.tab.active.label')  },
    { id: 'past'    as const, label: t('profile.tab.past.label')    },
    { id: 'spots'   as const, label: `♥ ${t('profile.tab.spots')}`  },
    { id: 'journey' as const, label: `✦ ${t('profile.tab.journey')}` },
  ];
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-8">
        ...
        <div className="flex justify-center gap-3 mt-4 flex-wrap">
          ...
          {profile.telegram_username && (
            <a
              ...
              aria-label={`Telegram @${profile.telegram_username.replace('@', '')}`}
            >
          ...
          {profile.instagram_handle && (
            <a
              ...
              aria-label={`Instagram @${profile.instagram_handle.replace('@', '')}`}
            >
      ...
      {profileIncomplete && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="flex-1 text-sm text-amber-800">{t('profile.incomplete.subtitle')}</span>
            <button
              type="button"
              onClick={() => setEditing(true)}
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        {[
          { labelKey: 'profile.stats.beaconsLit' as const, value: profile.beacons_lit },
          { labelKey: 'profile.stats.peopleMet'  as const, value: beacons.reduce((s, b) => s + b.join_count, 0) },
          { labelKey: 'profile.stats.savedSpots' as const, value: savedLocations.length },
        ].map(({ labelKey, value }) => (
          <div
            key={labelKey}
          ...
          >
            ...
            <p className="text-xs text-text-muted mt-1">{t(labelKey)}</p>
          </div>
        ))}
      </div>
          ...
          >
            {savedLocations.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="font-serif text-xl mb-2">{t('profile.spots.empty')}</p>
                <p className="text-sm text-text-muted mb-6">{t('profile.spots.empty.subtitle')}</p>
                <Link to="/map" className="rounded-full bg-text-main text-white px-6 py-2.5 text-sm font-medium">
                  {t('profile.spots.cta')}
                </Link>
              </div>
            ) : (
                  ...
                  >
                    ...
                    >
                      <div className="relative h-28">
                        <LocationImage src={loc.photo_url} alt={loc.name} category={loc.category} className="h-full w-full" />
                        <button
                          type="button"
                          onClick={async (e) => { e.preventDefault(); e.stopPropagation(); await toggleSave(loc.id); }}
          ...
          >
            {!history ? (
              <div className="flex items-center justify-center py-12 text-text-muted">
                ...
                <span className="text-sm">{t('profile.journey.loading')}</span>
              </div>
            ) : (
              <>
                ...
                {history.beacons.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-3xl mb-3">🕯</p>
                    <p className="font-serif text-xl mb-2">{t('profile.journey.empty.title')}</p>
                    <p className="text-sm text-text-muted mb-6">{t('profile.journey.empty.subtitle')}</p>
                    <Link to="/beacon/new" className="rounded-full bg-text-main text-white px-6 py-2.5 text-sm font-medium">
                      {t('profile.journey.cta')}
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {history.beacons.map((b) => {
                      const date = new Date(b.scheduled_at).toLocaleDateString(locale, {
                        ...
                        >
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-[#F0EDE8]">
                            {b.location?.photo_url ? (
                              <LocationImage src={b.location.photo_url} alt={b.location.name} category={b.location.category} className="w-full h-full" />
                            ) : (
